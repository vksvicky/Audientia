...
/// Audio visualizer implementation that produces FFT magnitude data for UI visualizers
public final class AudioVisualizer: AudioVisualizerProtocol, @unchecked Sendable {
    private let actor: VisualizerActor
    
    public init(config: AudioVisualizerConfig = AudioVisualizerConfig()) {
...
private actor VisualizerActor {
    private let config: AudioVisualizerConfig
    private var previousMagnitudes: [Float]
    private var history: [AudioVisualizerFrame]
    ...
    init(config: AudioVisualizerConfig) {
        self.config = config
    ...
    func latestFrame() -> AudioVisualizerFrame? {
        history.last
    }
    
    func recentFrames(limit: Int) -> [AudioVisualizerFrame] {
    ...
    private func validateInputs(audioData: [Float], sampleRate: Int, channels: Int) throws {
        guard fftSize >= 256, fftSize.nonzeroBitCount == 1 else {
            throw AudioVisualizerError.invalidFFTSize
        }
        guard sampleRate > 0 else {
            throw AudioVisualizerError.invalidSampleRate
        }
        guard channels > 0 else {
            throw AudioVisualizerError.invalidChannelCount
        }
    ...
    private func makeFrame(with magnitudes: [Float], sampleRate: Int) -> AudioVisualizerFrame {
    ...
    private func storeFrame(_ frame: AudioVisualizerFrame) {
