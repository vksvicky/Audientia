//
//  AudioVisualiserTapHelpers.swift
//  AudioCore
//
//  Helper methods for AudioVisualiserTap
//
...
// MARK: - AudioVisualiserTap Helper Methods Extension

extension AudioVisualiserTap {
    ...
    func scheduleAudioFile(
        ...
        playerNode.scheduleFile(audioFile, at: scheduleTime) {
            // File playback completed - reschedule to loop for continuous visualisation
            guard let engine = engineToCheck,
                  engine.isRunning,
                  nodeToReschedule.isPlaying else {
                Logger.audio.debug("AudioVisualiserTap: Skipping reschedule (engine stopped or node not playing)")
                return
            }
            ...
            Logger.audio.debug("AudioVisualiserTap: File rescheduled for continuous visualisation")
        }
    }
    ...
    func verifyPlayerNodeStatus(playerNode: AVAudioPlayerNode, engine: AVAudioEngine) {
        Task { @MainActor in
            ...
            if isNodePlaying && isEngineRunning {
                Logger.audio.info("AudioVisualiserTap: playerNode is playing, engine running, mixer volume: \(mixerVolume) - audio should flow through tap")
            } else {
                Logger.audio.warning("AudioVisualiserTap: playerNode.play() called but node playing: \(isNodePlaying), engine running: \(isEngineRunning), mixer volume: \(mixerVolume) - audio may not flow")
            }
        }
    }
    
    /// Schedule the entire file to be processed through the tap if needed
    /// The tap will capture audio data for visualisation
    /// - Parameters:
    ///   - playerNode: The player node
    ///   - audioFile: The audio file
    ///   - engine: The audio engine
    ...
    func scheduleFileIfNeeded(
        ...
        if needsReschedule {
            Logger.audio.info("AudioVisualiserTap: Node not playing, will reschedule file (likely after stop/pause)")
        }
        
        if shouldSchedule {
            Logger.audio.info("AudioVisualiserTap: Scheduling file for playback (startPosition: \(startPosition?.description ?? "nil"), needsReschedule: \(needsReschedule))")
            ...
            Logger.audio.info("AudioVisualiserTap: File scheduled successfully at time: \(scheduleTime?.description ?? "immediately")")
            ...
            if (startPosition == nil || startPosition == 0) && needsReschedule && !playerNode.isPlaying {
                ...
                Logger.audio.debug("AudioVisualiserTap: New file scheduled over paused node, will play immediately")
            }
        }
    ...
    func startPlayback(
        ...
        Logger.audio.info("AudioVisualiserTap: Starting playerNode (engine running: \(engine.isRunning), node playing: \(playerNode.isPlaying), scheduled: \(schedulingResult.shouldSchedule))")
        ...
        if !schedulingResult.shouldSchedule && schedulingResult.needsReschedule {
            Logger.audio.warning("AudioVisualiserTap: File was not scheduled but node needs reschedule - this should not happen")
        }
        ...
        Logger.audio.debug("AudioVisualiserTap: Player node started (scheduled: \(schedulingResult.shouldSchedule), volume: \(self.volume))")
        ...
        Logger.audio.info("AudioVisualiserTap: Started processing audio file for visualisation - file length: \(audioFile.length) frames, format: \(audioFile.processingFormat)")
    }
}
