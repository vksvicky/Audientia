...
/// TDD tests for the AudioVisualizer FFT feed
final class AudioVisualizerTests: XCTestCase {
    private var visualizer: AudioVisualizer!
    private var smoothingVisualizer: AudioVisualizer!
    private var historyVisualizer: AudioVisualizer!
    ...
    override func setUpWithError() throws {
        ...
        smoothingVisualizer = AudioVisualizer(
            config: AudioVisualizerConfig(
                fftSize: 1024,
                smoothingFactor: 0.5,
                historyLength: 4
        ...
        historyVisualizer = AudioVisualizer(
            config: AudioVisualizerConfig(
                fftSize: 512,
                smoothingFactor: 0.0,
                historyLength: 3
    ...
    func testProcessAveragesStereoChannels() async throws {
        ...
        let stereoVisualizer = AudioVisualizer(
            config: AudioVisualizerConfig(
                fftSize: fftSize,
                smoothingFactor: 0.0,
                historyLength: 4
        ...
        let monoVisualizer = AudioVisualizer(
            config: AudioVisualizerConfig(
                fftSize: fftSize,
                smoothingFactor: 0.0,
                historyLength: 4
    ...
    func testProcessValidatesInputs() async {
        await XCTAssertThrowsErrorAsync(
            try await visualizer.process(
                audioData: [],
                sampleRate: 44_100,
                channels: 1
            )
        ) { error in
            guard let visualizerError = error as? AudioVisualizerError else {
                return XCTFail("Expected AudioVisualizerError, got \(error)")
            }
            XCTAssertEqual(visualizerError, .invalidAudioData)
        }
        ...
        await XCTAssertThrowsErrorAsync(
            ...
            XCTAssertEqual(visualizerError, .invalidSampleRate)
        }
        ...
        await XCTAssertThrowsErrorAsync(
            ...
            XCTAssertEqual(visualizerError, .invalidChannelCount)
        }
    }
