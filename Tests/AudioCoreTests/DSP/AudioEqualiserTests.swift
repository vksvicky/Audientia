//
//  AudioEqualiserTests.swift
//  AudioCoreTests
//
//  TDD tests for 10-band parametric equaliser
//
...
/// TDD tests for AudioEqualiser
/// Following Right-BICEP principles
final class AudioEqualiserTests: XCTestCase {
    ...
    /// BDD: Given a new equaliser, when I check the bands, then all bands should be at 0.0 dB (flat)
    ...
    /// BDD: Given an equaliser, when I set a band gain, then that band should have the new gain
    func testSetBandGain() async throws {
        ...
        // Then
        let retrievedGain = try await equaliser.getBandGain(bandIndex)
        XCTAssertEqual(retrievedGain, gain, accuracy: 0.01, "Band gain should be set correctly")
    }
    
    /// BDD: Given an equaliser with modified bands, when I reset, then all bands should be flat
    ...
    /// BDD: Given an equaliser, when I process audio with flat bands, then output should match input
    ...
    /// BDD: Given an equaliser, when I set gain to maximum, then it should be clamped
    func testSetBandGainClampsToMaximum() async throws {
        ...
        // When
        try await equaliser.setBandGain(bandIndex, gain: extremeGain)
    ...
    /// BDD: Given an equaliser, when I set gain to minimum, then it should be clamped
    ...
    /// BDD: Given an equaliser, when I try to set gain for invalid band index, then it should throw error
    func testSetBandGainWithInvalidIndexThrowsError() async {
        ...
        // When & Then
        do {
            try await equaliser.setBandGain(invalidIndex, gain: 5.0)
            XCTFail("Should throw error for invalid band index")
        } catch let error as AudioEqualiserError {
            XCTAssertEqual(error, .invalidBandIndex(invalidIndex), "Should throw invalidBandIndex error")
        } catch {
            XCTFail("Should throw AudioEqualiserError, got: \(error)")
        }
    }
    
    /// BDD: Given an equaliser, when I process empty audio, then it should throw error
    ...
    /// BDD: Given an equaliser, when I process audio with invalid sample rate, then it should throw error
    ...
    /// BDD: Given an equaliser, when I set gain then reset, then gain should return to 0.0
    func testSetGainThenResetRoundtrip() async throws {
        ...
        // Then
        let resetGain = try await equaliser.getBandGain(bandIndex)
        XCTAssertEqual(resetGain, 0.0, accuracy: 0.01, "Gain should return to 0.0 after reset")
    }
    
    // MARK: - [C]ross-Checking Using Other Means
    
    /// BDD: Given an equaliser, when I check bands via multiple methods, then they should agree
    func testBandConsistencyAcrossMethods() async throws {
        ...
        // Then
        let bands = await equaliser.getBands()
        let directGain = try await equaliser.getBandGain(bandIndex)
    ...
    /// BDD: Given an equaliser, when I try to get gain for invalid band index, then it should throw error
    func testGetBandGainWithInvalidIndexThrowsError() async {
        ...
        // When & Then
        do {
            _ = try await equaliser.getBandGain(invalidIndex)
            XCTFail("Should throw error for invalid band index")
        } catch let error as AudioEqualiserError {
            XCTAssertEqual(error, .invalidBandIndex(invalidIndex), "Should throw invalidBandIndex error")
        } catch {
    ...
    /// BDD: Given an equaliser, when I process audio, then it should complete quickly
    ...
    /// BDD: Given an equaliser, when I set all bands to different gains, then all should be set correctly
    ...
    /// BDD: Given an equaliser, when I process mono audio, then it should handle correctly
    ...
    /// BDD: Given an equaliser, when I enable then disable, then processing should be bypassed
    func testEnableDisableBypass() async throws {
        // Given
        let sampleRate = 44100
        let channels = 2
        let audioData: [Float] = [0.5, -0.3, 0.8, -0.2]
        try await equaliser.setBandGain(5, gain: 6.0) // Set a band to non-zero
        
        // When - Disable
        await equaliser.setEnabled(false)
        let disabledProcessed = try await equaliser.process(
            audioData: audioData,
            sampleRate: sampleRate,
            channels: channels
        )
        ...
        let enabledProcessed = try await equaliser.process(
            audioData: audioData,
            sampleRate: sampleRate,
            channels: channels
        )
