//
//  AudioNormalisationBDDTests.swift
//  AudioCoreTests
//
...
@MainActor
final class AudioNormalisationBDDTests: XCTestCase {
    
    var normaliser: AudioNormaliser!
    ...
    func testUserNormalizesAudioLevelsAcrossLibrary() async throws {
        ...
        let quietGain = try await normaliser.analyzeNormalization(
        ...
        let loudGain = try await normaliser.analyzeNormalization(
            audioData: loudTrack,
            sampleRate: 44100,
            channels: 1,
            mode: .peak,
        ...
        let normalizedQuiet = try await normaliser.applyNormalization(audioData: quietTrack, gainDB: quietGain)
        let normalizedLoud = try await normaliser.applyNormalization(audioData: loudTrack, gainDB: loudGain)
        
        let quietPeak = await normaliser.calculatePeakLevel(audioData: normalizedQuiet, channels: 1)
        let loudPeak = await normaliser.calculatePeakLevel(audioData: normalizedLoud, channels: 1)
    ...
    func testUserNormalizesUsingPeakNormalization() async throws {
        ...
        let gainDB = try await normaliser.analyzeNormalization(
            audioData: audioData,
            sampleRate: 44100,
            channels: 1,
            mode: .peak,
        ...
        let normalized = try await normaliser.applyNormalization(audioData: audioData, gainDB: gainDB)
        let newPeak = await normaliser.calculatePeakLevel(audioData: normalized, channels: 1)
    ...
    func testUserNormalizesUsingRMSNormalization() async throws {
        ...
        let normalized = try await normaliser.applyNormalization(audioData: audioData, gainDB: gainDB)
        let newRMS = await normaliser.calculateRMSLevel(audioData: normalized, channels: 1)
    ...
    func testUserViewsPeakLevelOfAudio() async {
        ...
        let peakLevel = await normaliser.calculatePeakLevel(audioData: audioData, channels: 1)
    ...
    func testUserViewsRMSLevelOfAudio() async {
        ...
        let rmsLevel = await normaliser.calculateRMSLevel(audioData: audioData, channels: 1)
    ...
    func testUserNormalizesStereoAudio() async throws {
        ...
        let normalized = try await normaliser.applyNormalization(audioData: audioData, gainDB: gainDB)
        let newPeak = await normaliser.calculatePeakLevel(audioData: normalized, channels: 2)
    ...
    func testUserNormalizesMultipleTracks() async throws {
        ...
        for track in [track1, track2, track3] {
            ...
            let normalized = try await normaliser.applyNormalization(audioData: track, gainDB: gainDB)
            let peak = await normaliser.calculatePeakLevel(audioData: normalized, channels: 1)
    ...
    func testUserAdjustsNormalizationTarget() async throws {
        ...
        let gain1 = try await normaliser.analyzeNormalization(
            audioData: audioData,
            sampleRate: 44100,
            channels: 1,
            mode: .peak,
        ...
        let gain2 = try await normaliser.analyzeNormalization(
            audioData: audioData,
            sampleRate: 44100,
            channels: 1,
            mode: .peak,
