...
/// TDD tests for AudioNormalisation
/// Following Right-BICEP: Right, Boundary, Inverse, Cross-check, Error, Performance
@MainActor
final class AudioNormalisationTests: XCTestCase {
    ...
    func testCalculatePeakLevel() async {
        ...
        let peakLevel = await normaliser.calculatePeakLevel(audioData: audioData, channels: channels)
    ...
    func testCalculateRMSLevel() async {
        ...
        let rmsLevel = await normaliser.calculateRMSLevel(audioData: audioData, channels: channels)
    ...
    func testApplyThenRemoveGainRestoresOriginal() async throws {
        ...
        let normalized = try await normaliser.applyNormalization(audioData: original, gainDB: gainDB)
        let restored = try await normaliser.applyNormalization(audioData: normalized, gainDB: -gainDB)
    ...
    func testInvalidSampleRate() async {
        ...
        do {
            _ = try await normaliser.analyzeNormalization(
                audioData: audioData,
        ...
        } catch let error as AudioNormalisationError {
    ...
    func testPeakCalculationPerformance() async {
        ...
        let startTime = Date()
        _ = await normaliser.calculatePeakLevel(audioData: audioData, channels: 1)
    ...
    func testRMSCalculationPerformance() async {
        ...
        let startTime = Date()
        _ = await normaliser.calculateRMSLevel(audioData: audioData, channels: 1)
        let duration = Date().timeIntervalSince(startTime)
