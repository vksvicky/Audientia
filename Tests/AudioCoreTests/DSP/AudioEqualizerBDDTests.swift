//
//  AudioEqualizerBDDTests.swift
//  AudioCoreTests
//
//  BDD scenarios for 10-band parametric equalizer
//
...
/// BDD-style test scenarios for AudioEqualizer
/// Following user-centric "As a user, I want to..." format
final class AudioEqualizerBDDTests: XCTestCase {
    ...
    func testBoostBassFrequencies() async throws {
        // Given - I have an equalizer and audio playing
        let sampleRate = 44100
        let channels = 2
        let audioData: [Float] = [0.5, -0.3, 0.8, -0.2, 0.1, 0.9]
        
        // When - I boost the bass bands (31Hz, 62Hz, 125Hz)
        try await equalizer.setBandGain(0, gain: 6.0) // 31Hz
        try await equalizer.setBandGain(1, gain: 4.0) // 62Hz
        try await equalizer.setBandGain(2, gain: 3.0) // 125Hz
        ...
        let band0Gain = try await equalizer.getBandGain(0)
        let band1Gain = try await equalizer.getBandGain(1)
        let band2Gain = try await equalizer.getBandGain(2)
    ...
    func testReduceTrebleFrequencies() async throws {
        // Given - I have an equalizer
        // When - I reduce the treble bands (8kHz, 16kHz)
        try await equalizer.setBandGain(8, gain: -4.0) // 8kHz
        try await equalizer.setBandGain(9, gain: -6.0) // 16kHz
        
        // Then - The treble bands should be attenuated
        let band8Gain = try await equalizer.getBandGain(8)
        let band9Gain = try await equalizer.getBandGain(9)
    ...
    /// BDD: As a user, I want to reset all equalizer settings to flat
    ...
    /// BDD: As a user, I want to bypass the equalizer without losing my settings
    func testBypassEqualizerWithoutLosingSettings() async throws {
        // Given - I have configured the equalizer
        try await equalizer.setBandGain(5, gain: 6.0)
        let originalGain = try await equalizer.getBandGain(5)
        
        // When - I disable the equalizer
        await equalizer.setEnabled(false)
        let isEnabled = await equalizer.isEnabled()
        
        // Then - The equalizer should be disabled
        XCTAssertFalse(isEnabled, "Equalizer should be disabled")
        
        // And - My settings should be preserved
        let preservedGain = try await equalizer.getBandGain(5)
        XCTAssertEqual(preservedGain, originalGain, accuracy: 0.01, "Settings should be preserved when disabled")
        
        // When - I re-enable the equalizer
        await equalizer.setEnabled(true)
        
        // Then - The equalizer should be enabled again
        let reEnabled = await equalizer.isEnabled()
        XCTAssertTrue(reEnabled, "Equalizer should be enabled again")
    }
    
    /// BDD: As a user, I want to apply a preset equalizer curve (e.g., "Rock", "Jazz", "Classical")
    func testApplyPresetEqualizerCurve() async throws {
    ...
    func testFineTuneIndividualBands() async throws {
        ...
        try await equalizer.setBandGain(5, gain: targetGain)
        
        // Then - The band should have the exact gain I set
        let actualGain = try await equalizer.getBandGain(5)
        ...
        try await equalizer.setBandGain(5, gain: adjustedGain)
        
        // Then - The band should reflect the new gain
        let newGain = try await equalizer.getBandGain(5)
        XCTAssertEqual(newGain, adjustedGain, accuracy: 0.01, "Band should reflect adjusted gain")
    }
    
    /// BDD: As a user, I want to see all my equalizer band settings at once
    func testViewAllBandSettings() async throws {
        // Given - I have configured multiple bands
        try await equalizer.setBandGain(0, gain: 3.0)
        try await equalizer.setBandGain(3, gain: -2.0)
        try await equalizer.setBandGain(7, gain: 4.0)
    ...
    /// BDD: As a user, I want the equalizer to not modify audio when all bands are flat
    func testFlatEqualizerDoesNotModifyAudio() async throws {
        ...
        // When - I process the audio through the equalizer
        let processed = try await equalizer.process(
            audioData: audioData,
            sampleRate: sampleRate,
            channels: channels
    ...
    /// BDD: As a user, I want to process audio through the equalizer and hear the changes
    func testProcessAudioThroughEqualizer() async throws {
        // Given - I have audio and I've boosted the bass
        let sampleRate = 44100
        let channels = 2
        let audioData: [Float] = [0.5, -0.3, 0.8, -0.2, 0.1, 0.9]
        
        try await equalizer.setBandGain(0, gain: 6.0) // Boost bass
        ...
        XCTAssertNotEqual(processed, audioData, "Audio should be modified by equalizer")
        XCTAssertEqual(processed.count, audioData.count, "Output length should match input")
    }
}
