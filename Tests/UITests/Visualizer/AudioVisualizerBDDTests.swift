...
final class AudioVisualizerBDDTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        let config = AudioVisualizerConfig(fftSize: 1024, smoothingFactor: 0.8, historyLength: 60)
        mockVisualizer = MockAudioVisualizer(config: config)
    }
    ...
    func testUserSeesRealTimeSpectrum() async throws {
        ...
        // When - Audio is processed for visualization
        ...
        // Then - Should receive a visualization frame
        XCTAssertNotNil(frame, "Should return visualization frame")
    ...
    /// BDD: As a listener, when I view the visualizer, then I should see the dominant frequency
    func testUserViewsDominantFrequency() async throws {
        ...
        // When - User views the visualizer
    ...
    /// BDD: As a listener, when I view the visualizer, then I should see recent frame history
    ...
    func testFrameHistoryLimit() async throws {
        ...
        let config = AudioVisualizerConfig(fftSize: 1024, smoothingFactor: 0.8, historyLength: 60)
        let visualizer = MockAudioVisualizer(config: config)
        ...
        let allFrames = await visualizer.recentFrames(limit: 100)
    ...
    func testDifferentFFTSizes() async throws {
        ...
        let config = AudioVisualizerConfig(fftSize: 2048, smoothingFactor: 0.8, historyLength: 60)
        let visualizer = MockAudioVisualizer(config: config)
